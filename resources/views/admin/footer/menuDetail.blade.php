...
@section('content')
    ...
    <input type="hidden" id="getUrl" value="{{route('FootergetSubMenu')}}">
    <input type="hidden" id="updateUrl" value="{{route('FooterupdateSubMenu')}}">
...
@section('js')
    ...
    <script>
        let packageTable = $('#subMenuTable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{!! route('FooterfetchSubMenu', $menu -> id) !!}',
            columns: [
                {data: 'id'},
                {data: 'name'},
                {data: 'detail'},
